...
space = pyglet.image.load('resspritesspace.jpg')
player_image = pyglet.image.load('resspritesPlayerShip.png') # space_ship.png
player_laser = pyglet.image.load('resspriteslaser.png')
enemy_laser = pyglet.image.load('resspritesenemy_laser.png')
stats_bg_image = pyglet.image.load('resspritesstats_bg_white.png')

# loading sprite sheet animations for the ufo_head
ufo_head = pyglet.image.load('resspritesufoHead_Sh.png')
...
enemy_ship = pyglet.image.load('resspritesenemyShip_Sh01.png')
...
xplosion_image = pyglet.image.load('resspritesexplosion.png')
...
high_score = load_high_score(resscore.txt)
...
# explosion sound
explosion = pyglet.media.load('ressoundsexp_01.wav', streaming=False)
player_gun_sound = pyglet.media.load('ressoundsplayer_gun.wav', streaming=False)
...
def preload():
    global preloaded
    for i in range(2):
        bg_list.append(pyglet.sprite.Sprite(space, x=0, y=i1200))
    preloaded = True
...
def game_over()
    ...
    if score  int(high_score)
        save_high_score('resscore.txt', score)
...
if __name__ == __main__
    pyglet.clock.schedule_interval(update, 1.060)
    pyglet.app.run()